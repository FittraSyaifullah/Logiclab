...
const ASSEMBLY_SCHEMA = {
  ...
  required: ['content', 'AssemblyAndParts'],
  properties: {
    content: { type: 'string' },
    AssemblyAndParts: {
      type: 'object',
...
serve(async (req) => {
  ...
  try {
    ...
    const fullContext = (body?.context ?? {}) as FullContext
...
USER REQUEST:\n${userMessage}\n\nFULL PROJECT CONTEXT JSON (read-only):\n${JSON.stringify(fullContext)}`
    ...
    const assemblyPayload = parsed as {
      content?: string
    ...
    if (targetHardwareId) {
      const { data: updated, error } = await supabase
        ...
        .update({ assembly_parts: normalizedAssembly })
        .eq('id', targetHardwareId)
        .select('id')
        .single()
      if (error || !updated) {
        console.error('[EDGE:edit-assembly] Update hardware_projects failed', error)
        return new Response(JSON.stringify({ error: 'Failed to update assembly' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 500,
        })
      }
      ...
      if (existing?.id) {
        ...
        const { data: inserted, error } = await supabase
          .from('hardware_projects')
          .insert({ project_id: projectId, title: fullContext?.project || 'Hardware Project', assembly_parts: normalizedAssembly })
          .select('id')
          .single()
        if (error || !inserted) {
          console.error('[EDGE:edit-assembly] Insert hardware_projects failed', error)
          return new Response(JSON.stringify({ error: 'Failed to insert assembly' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 500,
          })
        }
    ...
    try {
      if (targetHardwareId) {
        await supabase.from('hardware_messages').insert([
          ...
          { hardware_id: targetHardwareId, role: 'assistant', content: normalizedAssembly.content || 'Assembly updated.' },
        ])
      }
    } catch (msgErr) {
