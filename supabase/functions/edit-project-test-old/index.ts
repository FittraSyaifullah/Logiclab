...
serve(async (req)=>{
  ...
  try {
    ...
    // Derive user from auth token; do not trust client-provided userId
    ...
    if (!Number.isFinite(currentBalance) || currentBalance < CREDIT_COST) {
      return new Response(JSON.stringify({ error: { code: 'INSUFFICIENT_CREDITS' } }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 402 });
    }
    ...
    // Deduct credits BEFORE persisting updates to prevent free writes on race
