...
function extractOutputText(data) {
  ...
  if (data && typeof data === 'object' && 'output_text' in data && data.output_text) {
    return data.output_text;
  }
  // Handle output array - find the message object (not reasoning)
  const output = data && typeof data === 'object' ? data.output : undefined;
  if (Array.isArray(output)) {
    const messageObj = output.find((item)=>item && item.type === 'message');
...
`;
    ...
    for (const job of jobs){
      ...
      try {
        ...
        console.log('[EDGE:hardware-initial-test] OpenAI response JSON received', {
          jobId: job.id,
          status: data?.status
        });
        ...
        let parsed;
        if (parsedDirect) {
          console.log('[EDGE:hardware-initial-test] Using parsed output directly', {
            jobId: job.id
          });
          parsed = parsedDirect;
        } else {
          const outputText = extractOutputText(data);
          console.log('[EDGE:hardware-initial-test] OpenAI output extracted', {
            jobId: job.id,
            hasOutputText: !!outputText,
            outputTextLength: outputText?.length
          });
          if (!outputText) {
            console.error('[EDGE:hardware-initial-test] No output text found in response', {
              jobId: job.id
            });
