...
serve(async (req) => {
  ...
  try {
    ...
    const system = `You are an AI engineer teaching non-technical users how to assemble hardware projects.
...
Generate comprehensive assembly instructions and parts list for this hardware project.`
    ...
    if (providedReportId) {
      const upd = await supabase.from('hardware_projects').update({ assembly_parts: { content: text, partsCount: 8, estimatedTime: '2-3 hours', difficultyLevel: 'Beginner' } }).eq('id', providedReportId).select().single()
      ...
      if (existingReport?.id) {
        const upd = await supabase.from('hardware_projects').update({ assembly_parts: { content: text, partsCount: 8, estimatedTime: '2-3 hours', difficultyLevel: 'Beginner' } }).eq('id', existingReport.id).select().single()
        if (upd.error) throw new Error(upd.error.message)
        reportData = upd.data as { id: string }
      } else {
        const insert = await supabase.from('hardware_projects').insert({ project_id: projectData.id, title: projectData.title || 'Hardware Project', assembly_parts: { content: text, partsCount: 8, estimatedTime: '2-3 hours', difficultyLevel: 'Beginner' } }).select().single()
    ...
    const debit = await debitIfUnpaid(supabase, ownerId, CREDIT_COST_HARDWARE_REPORT, 'hardware_report_assembly_parts', reportData!.id)
    if (!debit.ok) {
      console.warn('[EDGE-ASM] Post-success debit failed:', debit.error)
    }

    return new Response(JSON.stringify({ reportId: reportData?.id, content: text }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (error) {
