    ...
    for (const job of jobs){
      try {
        ...
        if (!resp.ok) {
          ...
          console.error('[EDGE:hardware-initial] OpenAI error:', errorText);
          throw new Error(`OpenAI error ${resp.status}: ${errorText}`);
        }

        const data = await resp.json();
        console.log('[EDGE:hardware-initial] Response received, status:', data?.status);

        // Prefer structured parsed output if present
        const parsedDirect = data?.output_parsed;
        let parsed: unknown;

        if (parsedDirect) {
          console.log('[EDGE:hardware-initial] Using parsed output directly');
          parsed = parsedDirect;
        } else {
          // Extract text from response (handles GPT-5 structure)
          const outputText = extractOutputText(data);
          if (!outputText) {
            console.error('[EDGE:hardware-initial] No output text found in response');
            throw new Error('Structured output missing payload');
          }
          console.log('[EDGE:hardware-initial] Extracted output text, length:', outputText.length);
          try {
            parsed = JSON.parse(outputText);
          } catch (e) {
            console.error('[EDGE:hardware-initial] JSON parse error:', e);
            console.error('[EDGE:hardware-initial] Output text preview:', outputText.slice(0, 500));
            throw new Error('Failed to parse structured JSON output');
          }
        }
        // Type assertion for parsed JSON structure
